...
동시성은 구현하기도 어렵고 디버깅하기도 까다롭다.  
`무엇` 과 `언제` 를 분리하는 일종의 decoupling 전략이다.  
이를 위해 동시성 코드가 일으키는 문제를 방어하는 원칙을 소개하고 있다.  

### 동시성 코드 작성 전략

1. 단일 책임 원칙  
동시성 관련 코드는 다른 코드와 분리하자.

2. 자료 범위 제한  
공유 객체를 사용하는 코드 영역 (`임계 영역`) 을 보호하자.
그리고 이러한 임계 영역을 줄여서 사이드이펙트를 최소화하자.  

3. 자료 사본을 사용하자  
공유 자료를 사용하지 않는 방법이 제일 좋다.  
스레드에서 공유 자료를 복사하여 사용하거나, 공유 객체를 읽기 전용으로 만든다.

4. 스레드는 가능하면 독립적으로 구현하자  
서로 독립된 스레드는 서로 동기화의 필요가 없기 때문에 안전하다.

### 동시성 실행 모델

1. 생산자-소비자  
생산자 스레드는 정보를 생성해 버퍼나 대기열(한정된 자원)에 넣는다. 소비자는 정보가 있을 때 읽는다.  
[해결책](https://ko.wikipedia.org/wiki/%EC%83%9D%EC%82%B0%EC%9E%90-%EC%86%8C%EB%B9%84%EC%9E%90_%EB%AC%B8%EC%A0%9C)


2. 읽기-쓰기  
읽기 스레드는 공유 자원을 사용하고, 쓰기 스레드는 자원을 갱신한다.    
[해결책](https://ko.wikipedia.org/wiki/%EB%8F%85%EC%9E%90-%EC%A0%80%EC%9E%90_%EB%AC%B8%EC%A0%9C)

3. 식사하는 철학자들  
[해결책](https://ko.wikipedia.org/wiki/%EC%8B%9D%EC%82%AC%ED%95%98%EB%8A%94_%EC%B2%A0%ED%95%99%EC%9E%90%EB%93%A4_%EB%AC%B8%EC%A0%9C)